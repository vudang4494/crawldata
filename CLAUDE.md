...
2. **A uv-workspace monorepo** implementing that spec: `apps/` (one package per stage S0–S6 + `service`) and `libs/common` (shared contract). **All seven stages S0–S6 have real pipeline logic, and the P1 features are in** (pure-Python core; heavy backends gated — see patterns below): quality classifier (§5.3, `cleaner/quality.py`, two gated backends — fasttext, and transformer = FineWeb2-HQ XLM-R embed + per-language `.pt` heads via `cleaner[quality]` extra; fail-closed when enabled without backend/model/head-per-lang), clustering BGE-M3→UMAP→HDBSCAN (§6, `profiler/cluster.py`, skip-reason when backends absent), Playwright JS-render (§3.2, `crawler/render.py`, auto-escalation + fail-closed `render:browser`), arq worker wiring (`service/worker.py` ↔ enqueue endpoints), and the **§14 agent intake layer** (`apps/agent` — {URL, need} → local LLM via OpenAI-compatible API analyses the S0 probe, asks the user clarifying questions, emits a Pydantic `DatasetPlan`, then runs S1→S5; the LLM only *proposes* — the plan is a safe subset of §9.3 with no gate-relaxing fields, and all gates stay deterministic), plus **Phase B synthetic QA** (§7.1, `builder/synth.py` — with `build.synth.enabled` the same local LLM turns each clean doc into instruction-QA pairs; provenance gains `synthetic`/`synth_model`, keeps the original `source_url`, and the license gate runs *before* generation so no inference is spent on unpublishable docs; per-doc LLM failures drop as `synth_failed`, never abort the run). The shared LLM client lives in `libs/common/llm.py` (`ChatLLM` + `extract_json_object`, httpx-gated) — stages must import it from common, never from `apps/agent`. Still pending: P2 (NeMo GPU dedup on A100, semantic dedup) and P3 service polish (job progress/eta, Grafana wiring).
...
| Run service API | `make run-service` (uvicorn `crawl_datasets_service.main:app`) |
| Run job worker | `make run-worker` (`arq crawl_datasets_service.worker.WorkerSettings`; needs Redis — `make docker-up`) |
...
- **Spec compliance on a file/dir:** `node .claude/skills/verify-design-spec/verify.mjs --target <path>` (exit 0 clean, 1 warn, 2 error). Run before merging anything touching S0–S6. Adding a rule to `verify.mjs` requires a paired PASS/FAIL case in `tests/test_verifier.py` (build bad fixtures by string concatenation so the test file doesn't match its own rule).
...
- **uv workspace:** `libs/common` (shared contract) + `apps/{probe,crawler,extractor,cleaner,profiler,builder,integrator,agent,service}`. Each pipeline app is an independent package exposing a `crawl-datasets-<x>` console script (`__main__.py`); `agent` (§14) is the LLM intake layer (`crawl-datasets-agent --url --need --out`, plus `/agent/sessions` endpoints — `IntakeSession` state machine is shared by CLI/API/tests, LLM injectable as a plain callable); `service` is a FastAPI + arq app (no CLI) that additionally depends on crawler/builder/integrator/agent to execute jobs — its enqueue endpoints (`main.py`) dispatch to worker functions (`worker.py`) by string name, and `tests/test_service.py` guards that mapping (fake pool injected via `app.state.arq_pool`, no Redis needed). Every cross-member dependency resolves via `[tool.uv.sources]` workspace entries at the root, not PyPI. Root `pyproject.toml` is a **virtual** workspace root (`[tool.uv.workspace]`, no `[project]`).
...
- **Specialized modules** hold the pure functions (e.g. cleaner: `normalize`/`lid`/`filters`/`dedup`/`pii`/`decontam`/`quality`; probe: `probe.py`; crawler: `frontier`/`render`; profiler: `profile`/`cluster`). Keep them pure and unit-testable.
- **Gated optional backend** — the load-bearing pattern. Heavy/optional deps (httpx, trafilatura, pymupdf, datatrove, fasttext/GlotLID, Presidio, pyarrow, playwright, umap/hdbscan, sentence-transformers) are imported behind `try/except ImportError` into an `_x: Any = None` module global, with a **pure-Python fallback** (stdlib HTML stripper, VN/EN LID heuristic, regex-only PII, JSONL-instead-of-Parquet). This keeps the core runnable and every test green with zero heavy installs. Never add a hard top-level import of a heavy dep; gate it and provide a fallback (and add it to the mypy overrides). Two sanctioned behaviors when a backend is missing: **fallback silently with equivalent output** (extractor, PII) or **fail loudly/visibly** — `SystemExit`/`RuntimeError` when the config explicitly demands the backend (`render: browser`, `clean.quality.enabled`), a recorded skip-reason when the stage is advisory (`profile.cluster` → `clustering_skipped`). Never skip silently.
