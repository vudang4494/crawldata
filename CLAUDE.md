...
2. **A uv-workspace monorepo** implementing that spec: `apps/` (one package per stage S0–S6 + `service`) and `libs/common` (shared contract). **All seven stages S0–S6 have real pipeline logic, and the P1 features are in** (pure-Python core; heavy backends gated — see patterns below): quality classifier (§5.3, `cleaner/quality.py`, two gated backends — fasttext, and transformer = FineWeb2-HQ XLM-R embed + per-language `.pt` heads via `cleaner[quality]` extra; fail-closed when enabled without backend/model/head-per-lang), clustering BGE-M3→UMAP→HDBSCAN (§6, `profiler/cluster.py`, skip-reason when backends absent), Playwright JS-render (§3.2, `crawler/render.py`, auto-escalation + fail-closed `render:browser`), arq worker wiring (`service/worker.py` ↔ enqueue endpoints). Still pending: P2 (NeMo GPU dedup on A100, semantic dedup) and P3 service polish (job progress/eta, Grafana wiring).
